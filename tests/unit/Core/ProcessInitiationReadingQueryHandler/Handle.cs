...
public sealed class Handle
{
    ...
    [Fact]
    public async Task ValidQuery_ReadsInitiation()
    {
        var expected = Mock.Of<object>();

        var fixture = FixtureFactory.Create<IQuery, object>();

        fixture.InitiationReaderMock.Setup(static (handler) => handler.Handle(It.IsAny<IIsProcessInitiatedQuery>(), It.IsAny<CancellationToken>())).Returns(Task.FromResult(expected));
