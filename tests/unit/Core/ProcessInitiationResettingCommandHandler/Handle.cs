...
public sealed class Handle
{
    ...
    [Fact]
    public async Task ValidCommand_ResetsInitiation()
    {
        var fixture = FixtureFactory.Create<ICommand>();

        await Target(fixture, Mock.Of<ICommand>(), CancellationToken.None);

        fixture.InitiationResetterMock.Verify(static (handler) => handler.Handle(It.IsAny<IResetProcessInitiationCommand>(), It.IsAny<CancellationToken>()), Times.Once());
    }
